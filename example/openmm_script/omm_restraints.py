...
def restraints(system, crd, inputs):
    ...
    if inputs.fc_wall > 0:
        ...
        wallres = CustomExternalForce('fc_wall * (px^2); \
                                       px = min(0, delta); \
                                       delta = r - droff; \
                                       r = abs(x - x0);')
        wallres.addPerParticleParameter('fc_wall')
    ...
    if inputs.fc_com > 0:
        ...
        x0=inputs.com
        particles1=[]
        # Just choose backbone to avoid complication
        for line in open('restraints/prot_pos.txt', 'r'):
            segments = line.strip().split()
            atom1 = int(segments[0])
            state = segments[1]
            if state == 'BB':  particles1.append(atom1)
        comres.addGroup(particles1)
        comres.addBond([0], [k,x0])
